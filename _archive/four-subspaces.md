...
(2) there is a set $W' \subseteq W$ with $\lvert W'\rvert=n-m$ such that $U\cup W'$ spans $V$.
...
All together I have $\dim N(T) = k$ and $\dim \text{im}(T) = \lvert S\rvert = n-k$ so
$$
\dim N(T) + \rank T = n
$$
as desired.
...
$$
so not only is the intersection of $N(A)$ and $\R(A)$ trivial, but they are orthogonal to each other. Finally, let $B_N$ and $B_R$ be bases for $N(A)$ and $\R(A)$ respectively. Each basis by itself is linearly independent by definition and $b_N\perp b_R$ for all $b_N\in B_N$, $b_R \in B_R$. This means $B := B_N\cup B_R$ is a linearly independent set of size $n$ (since $\lvert B_N\rvert = n-r$ and $\lvert B_R\rvert = r$). The dimension of $V$ is $n$ so $\lvert B\rvert=n$ makes $B$ a basis. This shows that $\R(A)$ and $N(A)$ partition $V$ into two orthogonal pieces. $\C(A)$ and $N(A^T)$ analogously partition $W$ into two orthogonal pieces.
