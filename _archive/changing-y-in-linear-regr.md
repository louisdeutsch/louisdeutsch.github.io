...
For the second requirement, let $r := \norm{y - \hat y}$. I need $\norm{\tilde y} = \norm{y}$ and I have
...
so vectors of the form $[a \mid s \mid t]^T$ will project to $\hat y$, and these are all elements of the 2-dimensional affine subspace $\hat y + N(X^T)$. Next I need $\norm{\tilde y}$ preserved which means my $z$ have norm $r$, so if I place a sphere of radius $r$ centered at $\hat y$ and look for the intersection with $\hat y + N(X^T)$ I will find all my valid perturbations. This is shown in the picture below.
