---
layout: home
title: Home
---

Hello! I am Louis Deutsch, and welcome to my personal website!

I will be graduating with a PhD in statistics from Wharton, at the University of Pennsylvania, in August of 2026 (I successfully defended in April 2026). My advisor was Eugene Katsevich. 

My PhD work focused on statistical and computational challenges in analyzing single-cell CRISPR screens. CRISPR lets us precisely perturb a region of a cell's genome, so we can use this to understand what various regions of the genome do by comparing gene expressions between perturbed and unperturbed cells.

One problem I studied is the _variable perturbation effectiveness problem_: some genomic perturbations work better than others, so we can lose power by effectively mixing control group cells into our treatment group. This led to _Location tests with noisy proxies for latent variables_ by Louis Deutsch and Eugene Katsevich (2026), published [here](https://www.sciencedirect.com/science/article/pii/S0167715225002354).

Another major project was to benchmark (both statistically and computationally) the main available methods for the two key steps in analyzing a single-cell CRISPR screen: (1) guide RNA assignment, where we infer which perturbations each cell actually received; and (2) association testing, where we perform millions of hypothesis tests to determine which perturbations affect which gene expressions. This paper will be submitted soon.

I am also a core developer of [SCEPTRE](https://timothy-barry.github.io/sceptre-book/), our lab's toolkit for analyzing these datasets.


Before beginning this PhD in August 2022, I spent 6 years as a data scientist where I worked on projects including machine learning for targeted advertising; fraud detection; social network analysis; and survey design with inferential analyses. I obtained a BS in statistics (with honors) and mathematics, as well as an MS in statistics, all from the University of Wisconsin-Madison. 

I used to have a blog at jld-stats.com (now defunct). I am in the process of moving my old blog posts into the "archive" section of this website. 

I also used to be very active on the statistics Q&A website Cross Validated. My profile can be found here: [![jld](https://stats.stackexchange.com/users/flair/30005.png?theme=clean)](https://stats.stackexchange.com/users/30005/jld)

If you are interested in collaborating or discussing anything statistical, feel free to contact me at `louisdeutsch1023 at gmail dot com`. My full CV is available upon request.
